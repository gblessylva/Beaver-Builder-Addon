...
(function($) {
	...
	class HJDashboard {
		...
		handleEditGoal() {
			...
			// Save goal ID, API token to localStorage for iframe compatibility
			localStorage.setItem('hj_current_goal_id', this.goalData.id);
			
			// Save API token if available
			if (typeof hjGoalAjax !== 'undefined' && hjGoalAjax.apiToken) {
				localStorage.setItem('hj_api_token', hjGoalAjax.apiToken);
			}
